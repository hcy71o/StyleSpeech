...
def test(args, model, step):    
    # Get dataset
    data_loader = prepare_dataloader_vctk(args.test_path, "test.txt", batch_size=32, shuffle=False) 
    ...
    for i, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            mel_output, _, _, log_duration_output, f0_output, energy_output, src_mask, mel_mask, out_mel_len = model(
                            text, src_len, mel_target, mel_target_style, mel_len, D, None, None, max_src_len, max_mel_len)
            # Cal Loss
